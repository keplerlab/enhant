...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
                ...
                chrome.tabs.sendMessage(tabs[0].id, {msg: "capture-meeting-info"}, function(response) {
                  ...
                  saveMeetingNumber(response.data, function(){
                        ...
                        enhant_local_storage_obj.read_multiple([STORAGE_KEYS.settings], function(result){
                            ...
                            chrome.tabs.sendMessage(request.data, {action: "capture_mic_start", data: settings_data || {}}, function(mic){
                                if (settings_data){
                                    sendResponse({
                                        status: true,
                                        settings: settings_data,
                                        mic_settings: mic.data,
                                        mic_error: mic.error 
                                    });
                                }
                                else{
                    ...
                    if (match_results.length > 0){
                        ...
                        if (match_score >= CONFIG.voice.threshold){
                            ...
                            if (_CAPTURE_SCREENSHOT_STRINGS.indexOf(matches_with_string) !== -1) {
                                ...
                                chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                                    chrome.tabs.sendMessage(tabs[0].id, {cmd: "capture_screenshot_voice"});
                                });
                            }
                            else{

                                chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                                    chrome.tabs.sendMessage(tabs[0].id, {cmd: "bookmark_moment_voice"});
                                });
                            }
                        }
                    }
        ...
        if (request.msg == "transcription_realtime"){
            ...
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                chrome.tabs.sendMessage(tabs[0].id, {msg: "transcription_notes", data: request.data}, function(response) {});
            });
