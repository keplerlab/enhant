...
(function(global) {
	...
	var WSAudioAPI = global.WSAudioAPI = {
		...
		Streamer: function(config, socket) {
			...
			if (_this.config.server.elClass){

				document.querySelector("." + _this.config.server.elClass).addEventListener('click', function() {
	...
	WSAudioAPI.Streamer.prototype.start = function(onError) {
        ...
        this.socket.onmessage = function(message) {
        	...
        	var event = new CustomEvent(_this.config.server.event, {
				detail: message.data
			});
